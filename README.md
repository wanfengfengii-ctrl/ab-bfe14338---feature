  ...
  "observations": [
    {
      ...
      "candidates": [
        { "id": "t-a", "minDelay": 0, "maxDelay": 20, "cost": 3 },
        { "id": "t-b", "minDelay": 5, "maxDelay": 40, "cost": 1 }
      ]
    }
  ]
}
...
至多 **18 条**观测可在 `candidates` 中附加 **1–3 个**候选档位，每档含：

- 观测内唯一的档位编号 `id`（非空字符串）；
- 非负整数延迟闭区间 `0 ≤ minDelay ≤ maxDelay ≤ 10⁹`；
- 整数校正代价 `cost ∈ [1, 1_000_000]`。

**原区间始终是零代价档位**，无需（也不得）列入 `candidates`。

- **仅当原裁决不可行**时才发起校正；原裁决可行（唯一/多解）时候选字段不改变任何展示。
- 未声明任何候选档位的旧模型完全保持原裁决与展示。
- 求解对“每观测取原档或某个候选档”的**全部组合**做完整精确比较，
  按以下优先级取字典序全局最优：
  1. 总代价最小；
  2. 被改观测数最少；
  3. 改动项台账 `[观测索引, 档位编号]`（按观测索引升序）字典序最小。

  实现为带界分支限界的穷举（不是逐条贪心，也不是首个可行即止）：
  部分组合一旦在已选档位上出现负环即整支剪枝（补全只会增加约束），
  代价/改动数下界劣于在任最优时同样整支剪枝；界面列出组合空间、
  访问节点数、到达叶子数与两类剪枝次数，使最优性可审计。
- 结果逐条列出改动前后区间、档位编号与代价，并给出恢复后的
  **紧确偏移范围与完整端点见证**（见证中各观测使用校正后区间逐项核对）。
- 若所有组合都不可行，明确给出**无方案**并保留原闭合矛盾链。
